  ...
  ({ inPatient, outPatient, dental, maternity, optical, Members }) => {
    ...
    const generatePDF = () => {
      ...
      function checkValue(val) {
        if (val === 0) return "Not Applicable";
        else return val;
      }
      ...
      doc.setFontSize(18);
      doc.text("Selected Options: ", 10, 60);
      doc.setFontSize(12);
      ...
      rateOptions.forEach((option, index) => {
        ...
        doc.text(option.value, 90, y); // Adjust the x-coordinate for the values alignment
        y += 10;
      });

      doc.setFontSize(18);
      doc.text("Total amount: ", 10, 175);
      doc.text(
        `${(
