...
function IntersectionValueComponent({
  ...
  const getInitialIntersectionValue = (sheetNumber) => {
    switch (sheetNumber) {
      case 0:
        return 44500; // Initial value for sheetNumber
      case 1:
        return 53400;
      default:
  ...
  const [intersectionValue, setIntersectionValue] = useState(
    // Set the initial intersection value based on the sheetNumber
    ...
    if (
      ...
      async function fetchData() {
        try {
          ...
          // Pass the intersection value and unique identifier to the parent component
          onIntersectionValueChange(intersectionValue || 0, uniqueId);
        } catch (error) {
